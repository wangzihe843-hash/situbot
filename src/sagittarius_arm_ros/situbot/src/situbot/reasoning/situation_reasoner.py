...
Changes from v1 (inspired by V-CAGE, arXiv:2604.09036):
  - Stage 3 outputs qualitative zones instead of exact coordinates (SIII-A2)
  - ZoneMapper converts zones to coordinates programmatically
  - PlacementOptimiser refines positions via L-BFGS-B (SIII-A4)
  - Rejection sampling generates N candidates, keeps the best (SIII-C)
"""
...
class ArrangementResult:
    ...
    reasoning_trace: str = ""
    rejection_sampling_info: Optional[Dict] = None
...
class SituationReasoner:
    ...
    def reason_with_rejection_sampling(
        ...
        logger.info(f"Rejection sampling: generating {n_candidates} candidates...")
    ...
    def _compute_legacy_arrangement(
        ...
        for obj in objects:
            ...
            dim_lines.append(f"- {obj['name']}: {d.get('w', 0.1)}m wide x {d.get('d', 0.1)}m deep")
        object_dims = "\n".join(dim_lines)
